...
def tree_wsv(dataset, 
    ...
    a_z_dense,b_z_dense = utils.tree_init(A,B,
                                          K=tree_child,
                                          D=tree_depth,
                                          tree='cluster')#,cdist=[np.asarray(twd_a),np.asarray(twd_b)])

    sil_old = -0.1
    twd_a_in, twd_b_in = [None,None]
    best = 0

    # meta-iterations (re-create tree structure each time)
    for i in range(n_iter):
        ...
        z_bll = utils.difference_subset(newa,ij_a,jnp.zeros_like(newa),b_z_dense,'atree',save=False)
        z_all = utils.difference_subset(newb,ij_b,jnp.zeros_like(newb),a_z_dense,'btree',save=False)
        ...
        # solving linear system of equations -- iterate
        for k in tqdm(range(n_inner_iter)):
            if total_converge > epsilon:
                ...
                wva_all = jnp.linalg.lstsq(y_a_red, twd_b_phi2) # or lsq
        ...
        plt.legend()
        plt.savefig(f'image/convergence_iter_{i}.png',dpi=150)
        plt.show()
    ...
    return a_z_dense, b_z_dense, twd_a, twd_b
    
if __name__ == "__main__":
    dataset = utils.generate_torus_data(n_features=100,n_samples=100)
    a,b,c,d = tree_wsv(dataset, n_iter=3)
